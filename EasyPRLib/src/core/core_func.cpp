...
namespace easypr {
  Mat colorMatch(const Mat &src, Mat &match, const Color r,
    ...
    // H range of green (new energy plate)

    const int min_green = 50;
    const int max_green = 95;
  ...
  Color getPlateType(const Mat &src, const bool adaptive_minsv) {
    ...
    if (plateColorJudge(src, BLUE, adaptive_minsv, blue_percent) == true) {
      ...
      // 不再无条件回退蓝色；四种颜色都未命中时返回 UNKNOWN，由调用方决定
      // （字符分割处会回退 BLUE 以保证识别流程不中断）
      max_percent = blue_percent;
      max_color = BLUE;
      if (green_percent > max_percent) {
        max_percent = green_percent;
        max_color = GREEN;
      }
      if (yellow_percent > max_percent) {
